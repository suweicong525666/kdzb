...
Page({
    ...
    if(options.userid){
        ...
        this.login(options.userid);
            ...
            my.getAuthCode({
              ...
                 success: (result) => {
              //      console.log(result);
                  request('/api/v2/Login/GetAliUserSilent/GetAliUserSilent','GET',{auth_code:result.authCode,appid:'2021002142657620'}).then(res=>{
                    if(res.success){
  ...
  login(userid){
         ...
         my.getAuthCode({
			...
			   success: (result) => {
      //      console.log(result);
           request('/api/v2/Login/GetAliUserSilent/GetAliUserSilent','GET',{auth_code:result.authCode,puserid:userid,appid:'2021002142657620'}).then(res=>{
             if(res.success){
                console.log('后端返回个人信息',res.data.access_token);
                my.setStorageSync({
                      key: 'token',
